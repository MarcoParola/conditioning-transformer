...
class EarlyMultiplicationDETR(nn.Module):
    def __init__(self, args):
        super(EarlyMultiplicationDETR, self).__init__()
    ...
    def forward(self, x: Tensor, meta: Tensor) -> Dict[str, Union[Tensor, List[Dict[str, Tensor]]]]:
        ...
        # implementing the early multiplication strategy
...
class EarlyMultiplicationDETRWrapper(nn.Module):
    """ A simple EarlyMultiplicationDETR wrapper that allows torch.jit to trace the module since dictionary output is not supported yet """

    def __init__(self, early_mul_detr, postProcess):
        super(EarlyMultiplicationDETRWrapper, self).__init__()

        self.early_mul_detr = early_mul_detr
...
def buildInferenceModel(args, quantize=False):
    ...
    model = EarlyMultiplicationDETR(args).to(device)
    model.load_state_dict(torch.load(args.weight, map_location=device))

    postProcess = PostProcess().to(device)

    wrapper = EarlyMultiplicationDETRWrapper(model, postProcess).to(device)
    wrapper.eval()
