...
class EarlyAffineDETR(nn.Module):
    def __init__(self, args):
        super(EarlyAffineDETR, self).__init__()
    ...
    def forward(self, x: Tensor, meta: Tensor) -> Dict[str, Union[Tensor, List[Dict[str, Tensor]]]]:
        ...
        meta_sum = self.metaProjectionSum(meta)
        meta_mul = self.metaProjectionMul(meta)
        meta_sum = meta_sum.view(-1, *self.projection_size)
        meta_mul = meta_mul.view(-1, *self.projection_size)

        # implementing the early affine strategy
        features = (features * meta_mul) + meta_sum
...
class EarlyAffineDETRWrapper(nn.Module):
    """ A simple EarlyAffineDETR wrapper that allows torch.jit to trace the module since dictionary output is not supported yet """

    def __init__(self, early_affine_detr, postProcess):
        super(EarlyAffineDETRWrapper, self).__init__()

        self.early_affine_detr = early_affine_detr
...
def buildInferenceModel(args, quantize=False):
    ...
    model = EarlyAffineDETR(args).to(device)
    model.load_state_dict(torch.load(args.weight, map_location=device))

    postProcess = PostProcess().to(device)

    wrapper = EarlyAffineDETRWrapper(model, postProcess).to(device)
    wrapper.eval()
