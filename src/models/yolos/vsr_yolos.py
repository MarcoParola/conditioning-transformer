...
from src.models.vsr.basicvsr_arch import BasicVSR


class VSRYolos(nn.Module):
    def __init__(self, args):
        ...
        self.vsr_enhancer = BasicVSR(spynet_path=args.spynet.weights)
        if args.spynet.freeze:
            for param in self.vsr_enhancer.parameters():
...
def main(args):
    ...
    for j in range(4):
        print(j)
        img = torch.randn(16, 3, 1, 72, 96)
        rlt = model(img)
        print(rlt['class'].size(), rlt['bbox'].size())
